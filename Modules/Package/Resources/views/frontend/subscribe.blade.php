...
<div class="inner-page">
  <div class="container">
    <form action="{{ route('frontend.packages.subscribe',$packagePrice->id) }}" method="post" id="updateForm">
      @csrf
      <input type="hidden" name="start_date" value="{{ Session::has('start') ? Session::get('start') : date('Y-m-d',strtotime($minDate)) }}">
      <div class="row cart-page {{locale() == 'ar' ? 'remove-flex' : '' }}">

        <div class="col-md-8 {{locale() == 'ar' ? 'float-right' : 'float-left'}}">
          <div class="checkout-block">
            ...
            <div class="row" id="theApp">
                ...
                <div class="form-group col-md-12" v-if="!get_cities_loader">
                ...
                <div class="form-group">
                    <input type="text" class="form-control" name="gada" placeholder="{{__('Gada Address')}}">
                </div>
                <div class="form-group">
            ...
            <div class="choose-time active hidden">
        ...
        <div class="col-md-4 {{locale() == 'ar' ? 'float-left' : 'float-right'}}">
          <div class="order-summery">
            ...
            <div class="cart-summery-content">
                  ...
                  <div id="couponForm">
                    <div class="d-flex promo-code align-items-center justify-content-between">
                      <div class="d-flex mb-20 promo-code align-items-center">
                        ...
                        <div id="coupon_success" class="coupon_success"></div>
              ...
              <button class="btn theme-btn d-block mt-20 submit" id="submit" type="submit">
                ...
                <span class="btn_loader" style="display: none"><x-front-btn-loader/></span>
              </button>
            </div>
          </div>
        </div>
...
const { createApp } = Vue;
 let app = createApp({
  ...
  app.getCities(1)
  let startRequest=  "{{Session::has('start')?Session::get('start') : ''}}";
  $('#datepicker').datepicker('setDate',startRequest != '' ? new Date(startRequest) : new Date("{{date('Y-m-d',strtotime($minDate))}}"))
  if(startRequest == ''){
   startRequest ="{{date('Y-m-d',strtotime($minDate))}}";
  }
$('#datepicker').datepicker('option', {
  minDate: new Date(startRequest),
});
