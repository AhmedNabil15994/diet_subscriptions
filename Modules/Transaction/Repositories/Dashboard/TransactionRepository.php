...
class TransactionRepository
{
    ...
    public function QueryTable($request)
    {
        ...
        if (isset($request['result']) && $request['result'] == 'CAPTURED'){
            $query->whereHas('Subscription',function ($whereHasQuery){
                $whereHasQuery->where('paid','paid');
            });
        }else if ($request['result'] == 'PENDING'){
            $query->whereHas('Subscription'  , function ($whereHas){
                $whereHas->where('paid','failed');
            });
        }
